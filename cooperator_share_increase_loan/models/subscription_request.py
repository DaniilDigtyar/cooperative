...
class SubscriptionRequest(models.Model):
    ...
    def _onchange_type(self):
        if self.type == "increase" and self.share_product_id:
    ...
    def create_permanent_loan(self):
        self.ensure_one()
        if self.type == "increase" and not self.loan_id:
            product = self.share_product_id
    ...
    def write(self, vals):
        ...
        if vals.get('state') == 'paid':
            for subscription in self:
                if subscription.type == 'increase' and not subscription.loan_id:
                    _logger.info("Creating loan after subscription paid: %s", subscription.id)
                    subscription.create_permanent_loan()
                    if subscription.loan_id:
                        subscription.loan_id.compute_lines()
                        _logger.info("Loan created and computed: %s", subscription.loan_id.id)
        return res
